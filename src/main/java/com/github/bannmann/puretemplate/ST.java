...
import com.github.bannmann.puretemplate.debug.ConstructionEvent;
...
public class ST
{
    ...
    /**
     * Inject an attribute (name/value pair). If there is already an attribute with that name, this method turns the
     * attribute into an {@link AttributeList} with both the previous and the new attribute as elements. This method
     * will never alter a {@link List} that you inject. If you send in a {@link List} and then inject a single value
     * element, {@code add} copies original list and adds the new value. The attribute name cannot be null or contain
     * '.'.
     * <p>
    ...
    /**
     * Split {@code aggrName.{propName1,propName2}} into list {@code [propName1, propName2]} and the {@code aggrName}.
     * Spaces are allowed around {@code ','}.
     */
    public synchronized ST addAggr(String aggrSpec, Object... values)
    {
        ...
        if (values.length != propNames.length)
        {
            throw new IllegalArgumentException(
                "number of properties and values mismatch for aggregate attribute format: " + aggrSpec);
        }
        int i = 0;
